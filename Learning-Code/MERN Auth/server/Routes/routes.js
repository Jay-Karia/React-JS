...
const PASSPORTSECRET = 'superPassportSecret'
...
router.post('/register', async(req, res) => {
    ...
    if (takenEmail.length !== 0 || takenName.length !== 0) {
        res.status(400).json({ msg: "Username or email has already been taken" })
    } else {
...
router.post('/login', async(req, res) => {
    ...
    User.findOne({
        ...
        if (dbUser) {
            // checks whether a user exsists in db with matching email
            bcrypt.compare(userLoggingIn.password, dbUser.password).then((isCorrect) => {
                ...
                jwt.sign(
                    payload,
                    JWT_SECRET, { expiresIn: 86400 }, // in 1 day
                    (err, token) => {
